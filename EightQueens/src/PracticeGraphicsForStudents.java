...
public class PracticeGraphicsForStudents {
	...
	private JPanel buildGridPanels() {
		...
		for (int r = 0; r < ROWS; r++) {
			for (int c = 0; c < COLS; c++) {
				...
				int x = 0;
				for (int i = WINDOW_SIZE / 4; i <= FrameBoi.WINDOW_SIZE / 4 + 160; i += 30) {
					for (int j = WINDOW_SIZE / 4; j <= FrameBoi.WINDOW_SIZE / 4 + 160; j += 30) {
						if (x++ % 2 == 0) {
				...
				MyPanel m = new MyPanel(bg, (char) ('a' + r * COLS + c) + "");
	...
	private void updatePanel(int r, int c, String letter) {
		...
		// window.repaint(); // because the set fields update the graphics, this isn't
		// needed
	}
